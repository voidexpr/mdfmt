...
Pages generated by `serve` and `build` register a service worker at the site
root. Every request goes to the network first and the response replaces the
cached copy, so an online reader always sees the latest version. When the
network is unavailable, visited pages are served from the cache, and an
unvisited page lands on an offline page listing the cached documents. A
small "Offline" pill appears in the toolbar while the device is offline.

Directory pages have a button that caches every page below the folder, with
the file count and total size shown before and after. Once cached, the same
button removes the folder's copies. The cached state is always read from
the cache itself, since the device may evict it. The site index behind the
button is `_mdfmt/site.json` in a build and `.mdfmt/site.json` in `serve`;
a build lists each page's referenced images, `serve` lists pages only.

Offline support needs a secure context: HTTPS in production, or
`127.0.0.1` locally. A rotated path token is a new site with an empty cache,
so an installed home-screen app is best served from a stable `--path-token`.
`save` output has no service worker.
