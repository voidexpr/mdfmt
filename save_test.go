...
func TestStandaloneRenderingPrivacyAndFeatures(t *testing.T) {
	...
	for _, want := range []string{
		...
		`id="detail"`,
		`id="appendix"`,
		`class="page-meta"><time datetime="` + modified.Format(time.RFC3339) + `" data-relative-time data-relative-style="compact">3h ago</time>`,
		`<span class="meta-detail"> · ` + modified.Local().Format("Jan 2, 2006 3:04 PM") + `</span> · `,
