...
func TestPathTokenRoutingAndHTMLPrivacy(t *testing.T) {
	...
	asset := request(t, server, "/"+token+"/.mdfmt/style.css?v=8")
...
func TestFileAndDirectoryRouting(t *testing.T) {
	...
	t.Run("root directory", func(t *testing.T) {
		...
		for _, want := range []string{"Name", "Last modified", "Ago", "Size", "Title", `data-sort-key="name"`, `data-sort-key="modified"`, `data-sort-key="size"`} {
...
func TestServedStylesheetIncludesSyntaxThemes(t *testing.T) {
	root := t.TempDir()
	response := request(t, testServer(t, root), "/.mdfmt/style.css?v=8")
	if response.Code != http.StatusOK {
		t.Fatalf("status = %d", response.Code)
	}
	body := response.Body.String()
